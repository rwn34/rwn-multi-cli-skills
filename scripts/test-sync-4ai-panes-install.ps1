...
#
# Builds a throwaway git repo (primary checkout on main) + a bare "origin"
# remote (so the ancestor guard's `origin/main` resolves) + a linked worktree
...
#   (b) primary, non-main    -> REFUSED, exit 0, branch named
#   (c) primary, detached HEAD -> REFUSED, exit 0
#   (d) primary + main, HEAD is an ancestor of origin/main -> PROCEEDS,
...
#   (i) local main AHEAD of origin/main (unpushed commit) -> REFUSED
...
#   (k) origin/main unresolvable (no 'origin' remote at all) -> REFUSED
...
# -- sandbox: primary repo on main + linked worktree + install targets --
...
try {
    ...
    # present), one commit on main. Also build a bare "origin" remote and
    # point the repo at it so the ancestor guard's `origin/main` resolves -
    # without this every primary+main run would hit the fail-closed
    # "origin/main UNRESOLVABLE" path instead of the happy path.
    ...
    Invoke-Git $work @('init', '--bare', '-b', 'main', $originDir) | Out-Null
    ...
    # (b) primary checkout, non-main branch -> REFUSED
    Invoke-Git $repo @('checkout', '-b', 'feature') | Out-Null
    $tB = New-Target 'install-b'
    $rB = Invoke-Sync $repoSync $tB
    Assert-Equal 0 $rB.Code 'b1: non-main primary exits 0'
    Assert-Contains $rB.Text 'REFUSED' 'b2: non-main primary prints REFUSED'
    ...
    # (d) primary checkout on main, HEAD is an ancestor of origin/main
    # (it IS origin/main, just pushed above) -> PROCEEDS
    ...
    # (f) -Force switch overrides the non-main refusal
    ...
    # (i) local main AHEAD of origin/main (an unpushed commit) -> REFUSED
    ...
    # Seed a known-good prior deploy first (HEAD == origin/main here).
    ...
    # Now advance local main with an UNPUSHED commit - origin/main does
    ...
    & git init -b main $noOrigin 2>$null | Out-Null
