...
================================================================================
 ...
 ANCESTOR GUARD (a THIRD property - added 2026-07-14 - on top of primary+main):
   Being on a LOCAL branch literally named 'main' is not the same claim as
   "this commit is reachable from origin/main". A local main that was
   committed to directly, merged into locally, or simply never pushed would
   pass the primary+main check above and still deploy code the fleet's peer
   review / CI gate never saw. So once primary+main passes (or is forced),
   a second, independent check runs: `git fetch origin main` (best-effort,
   see below) then `git merge-base --is-ancestor HEAD origin/main`. HEAD not
   ...
   untouched (no partial deploy). `origin/main` unresolvable (no remote, no
...
# --- Ancestor guard: HEAD must be reachable from origin/main --------------
...
# origin/main actually contains - a local-only commit or merge would pass
...
if (-not $forced) {
    ...
    # origin/main ref-tip is already known locally, and fails closed on its
    # own if that is stale, missing, or the repo has no 'origin' at all.
    $prevEAP2 = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    & git -C $SourceDir fetch origin main 2>$null | Out-Null
    $ErrorActionPreference = $prevEAP2

    $isAncestor = $false
    $prevEAP3 = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    & git -C $SourceDir rev-parse --verify --quiet 'origin/main' 2>$null | Out-Null
    $originMasterResolves = ($LASTEXITCODE -eq 0)
    if ($originMasterResolves) {
        & git -C $SourceDir merge-base --is-ancestor HEAD origin/main 2>$null
    ...
    if ($allowUnmerged) {
        if (-not $originMasterResolves) {
            Write-Warning "sync-4ai-panes-install: UNMERGED-ALLOWED (RWN_4AI_ALLOW_UNMERGED=1) - origin/main is UNRESOLVABLE, provenance of commit=$commit cannot be verified at all. Deploying anyway because the escape hatch is set."
            $unmergedAllowedUsed = $true
        } elseif (-not $isAncestor) {
            Write-Warning "sync-4ai-panes-install: UNMERGED-ALLOWED (RWN_4AI_ALLOW_UNMERGED=1) - commit=$commit on branch=$branch is NOT an ancestor of origin/main. Deploying UNMERGED code to the live install because the escape hatch is set."
            $unmergedAllowedUsed = $true
        }
    } elseif (-not $originMasterResolves) {
        Write-Line "sync-4ai-panes-install: REFUSED - origin/main is UNRESOLVABLE (no remote, offline, or no such ref) so commit=$commit's provenance cannot be verified. Fail-closed. Override: RWN_4AI_ALLOW_UNMERGED=1."
        Write-SyncLog -Result 'refused-unverifiable-origin' -Actions @()
        exit 0
    } elseif (-not $isAncestor) {
        Write-Line "sync-4ai-panes-install: REFUSED - commit=$commit on branch=$branch is NOT an ancestor of origin/main. A locally-advanced or unpushed 'main' does not license a deploy. Override: RWN_4AI_ALLOW_UNMERGED=1."
