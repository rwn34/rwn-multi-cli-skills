...
Entry files in `.ai/activity/entries/*.md` that claim file changes should name
the touched paths and, when claiming a specific construct landed, include a grep
snippet or the resulting line. Lighter than handoffs because activity entries
are summary-level and written one per file — but readers (humans and other CLIs)
