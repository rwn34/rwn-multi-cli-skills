...
- The diff of `dispatch-handoffs.sh` for each of the three fixes.
- Raw output of the full `bash .ai/tests/test-dispatch-worktree.sh` run, including
  the new/rewritten cases, with the PASS/FAIL tally. **Execution evidence, not
  inspection** — do not claim green without pasting the run.
- Confirmation that `v4-3` now asserts the HOLD, quoted verbatim.
- Anything in ADR-0015 you think is wrong. The ADR is a decision, not a
  monologue — if the ancestor rule or the hard-gate list is impractical, say so in
  a `## Blocker` rather than implementing something you believe is broken.
