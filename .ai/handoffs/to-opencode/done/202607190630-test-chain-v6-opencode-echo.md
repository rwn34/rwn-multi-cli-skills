...
## Goal
Echo test: confirm the snapshot-copy dispatcher can drive an opencode child in a
parallel fan-out.

## Steps
1. Write a return handoff to
   `.ai/handoffs/to-claude/open/202607190630-test-chain-v6-opencode-return.md`
   with `Sender: opencode`, `Recipient: claude`, `Status: OPEN`, and a `## Result`
   section containing the single line: `opencode-echo`.
2. Self-retire this handoff: set `Status: DONE` and move it to
   `.ai/handoffs/to-opencode/done/`.

## Verification
- (a) `ls .ai/handoffs/to-claude/open/202607190630-test-chain-v6-opencode-return.md` exists.
- (b) This handoff appears in `.ai/handoffs/to-opencode/done/` with `Status: DONE`.
