...
Evidence: VERIFIED (git rev-parse HEAD -> 9797a1f16f70eff2edc9b714945db6ff31f20218; ls .ai/handoffs/to-opencode/open -> empty before this file)

## Goal

Echo back to the chain aggregator with marker `CHAINV7-OPENCODE-b40e6c`, proving
the claude → opencode leg of the v7 auto-dispatch chain works end to end.

## Steps

1. Write a return handoff at
   `.ai/handoffs/to-claude/open/20260721T085700Z-test-chain-v7-opencode-return.md`
   with this status block (verbatim, adjusting only `Created:`):
       ...
       Evidence: VERIFIED (echo child executed by opencode auto pane)

   Body must contain exactly one line reading:

       Marker: CHAINV7-OPENCODE-b40e6c

   Do NOT aggregate, do NOT wait for sibling children (kimi, kiro), and do NOT
   emit any final handoff. One file, then stop. No git/GitHub operations are in
   scope for this test — writing the return file is the whole task.
2. Prepend an entry to `.ai/activity/log.md` using identity `opencode`.
...
- (b) `grep -n "CHAINV7-OPENCODE-b40e6c" .ai/handoffs/to-claude/open/20260721T085700Z-test-chain-v7-opencode-return.md`
...
- (c) confirmation this file was moved to `to-opencode/done/`
