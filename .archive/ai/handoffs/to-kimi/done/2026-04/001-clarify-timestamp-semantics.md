# Clarify timestamp semantics in Kimi's AI contract
Status: DONE
Sender: claude-code
Recipient: kimi-cli
...
## Goal
Add an explicit line to `.kimi/steering/00-ai-contract.md` saying the activity-log
timestamp is the wall-clock time when you **prepend** the entry (i.e. when the work
finishes), not when it started. Prior entries have shown timestamps that are earlier
than entries above them (Kimi's 08:46 appeared at the top, below entries timestamped
later) — the current contract is silent on which point in time the timestamp refers
to, so this ambiguity should be closed.

## Current state
`.kimi/steering/00-ai-contract.md` describes the log entry format:
...
`AGENTS.md`, `/CLAUDE.md`) with the same clarification. This handoff just asks you to
add the matching note inside Kimi's own always-loaded contract so the rule is visible
at every Kimi turn without needing the activity log to be consulted.
...
5. (Optional) If you disagree with the wording or want to adapt it for Kimi-specific
   nuances, that's fine — note the final wording in your activity log entry.

## Verification
- (a) `.kimi/steering/00-ai-contract.md` contains the new paragraph (verbatim from
...
## Activity log template
    ## YYYY-MM-DD HH:MM — kimi-cli
    - Action: Clarified timestamp semantics in .kimi/steering/00-ai-contract.md per handoff 001 — timestamp = wall-clock at prepend (finish time), not start. Prepend order is authoritative.
...
## When complete
Claude will read `.kimi/steering/00-ai-contract.md` to validate. On success, this
file moves to `.ai/handoffs/to-kimi/done/`.
