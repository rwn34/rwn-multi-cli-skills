...
11. All agents classify actions by **autonomy tier** (operating-prompt §8): Tier A proceeds, Tier B acts-then-notifies, Tier C asks first. Commits/pushes on feature branches are Tier A; **merge to main, PR open/merge, branch/worktree cleanup, ADR authorship/amendment, and staging deploy are Tier B** (fleet-executed, notify after); **production deploy, publish/tag, and destructive ops on shared history** are Tier C.
