---
name: self-grep-verify
description: A discipline for grounding claims of completed work in the actual working tree — before reporting done, grep the tree for each concrete claim and paste 1-3 matching lines as evidence. Activate before writing a completion handoff in `.ai/handoffs/`, before prepending an activity log entry that claims file changes, or before telling the user "I fixed X" / "I added Y" / "I wrapped N sites". Catches junior-engineer optimism (claim overstates completeness) before it lands. Trigger phrases include "completion handoff", "activity log entry", "claim", "grep verify", "before reporting done". Applies equally to Claude Code, Kimi CLI, and Kiro CLI.
---

<!-- SSOT: .ai/instructions/self-grep-verify/principles.md — regenerate via .ai/sync.md -->
